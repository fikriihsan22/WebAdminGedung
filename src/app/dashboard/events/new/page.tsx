...
import { listActiveBookingSpaces } from "@/lib/server/buildings";

import { EventForm } from "./event-form";

export default async function NewEventPage() {
  const user = await requireBuildingAdmin();
  const spaces = await listActiveBookingSpaces(user.buildingId);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader><h2 className="font-semibold">Data acara</h2></CardHeader><CardContent><EventForm spaces={spaces.map((space) => ({ id: space.id, name: space.name }))} /></CardContent></Card>
    </div>
  );
}
